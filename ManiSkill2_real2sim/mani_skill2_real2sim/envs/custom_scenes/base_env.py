...
class CustomSceneEnv(BaseEnv):
    ...
    def __init__(
            ...
            urdf_version: str = "",
            ###ai
            # robot_uids: Optional[Union[str, List[str]]] = None,
            **kwargs
        ):
        # Assets and scene
        # kwargs.pop('robot_uid', None)
        ###ai
        # kwargs.pop('robot_uids', None) # BaseEnv 不认识 robot_uids
        # kwargs.pop('robot_uid', None)  # BaseEnv 不认识 robot_uid
        # kwargs.pop('robot_init_qpos_noise_scale', None)
        ###ai
