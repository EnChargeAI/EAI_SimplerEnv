...
class GraspSingleInSceneEnv(CustomSceneEnv):
    ...
    def _initialize_actors(self):
        # 在 _initialize_actors 方法开头
        placed_distractor_xys = [] 
        ...
        if len(self.distractor_objs) > 0:
            print(f"DEBUG: Placing {len(self.distractor_objs)} distractor objects.")
...
###ai
                # 在 for distractor_obj in self.distractor_objs: 循环内，生成 distractor_init_xy 的部分
                if distractor_init_xy is None:
                    print(f"DEBUG: {distractor_obj.name} init_xy is None, generating a new one.")
                    max_placement_attempts = 100
                    for attempt in range(max_placement_attempts):
                        candidate_xy = obj_init_xy + self._episode_rng.uniform(-0.3, 0.3, [2])
                        candidate_xy = np.clip(candidate_xy, [-0.50, 0.05], [-0.1, 0.35])

                        # 检查与目标物体的距离
                        if np.linalg.norm(candidate_xy - obj_init_xy) <= 0.25: # 太近目标物体
                            continue

                        # 检查与已放置干扰物的距离
                        is_too_close_to_other_distractor = False
                        for placed_xy in placed_distractor_xys:
                            if np.linalg.norm(candidate_xy - placed_xy) <= 0.10: # 假设干扰物间最小距离0.1米
                                is_too_close_to_other_distractor = True
                                break

                        if not is_too_close_to_other_distractor:
                            distractor_init_xy = candidate_xy
                            break
                    else: # for-else: 如果循环正常结束（没有break），说明没找到合适位置
                        print(f"WARN: Could not place {distractor_obj.name} sufficiently far from others after {max_placement_attempts} attempts. Placing at last candidate.")
                        distractor_init_xy = candidate_xy # 或者采取其他策略

                placed_distractor_xys.append(distractor_init_xy) # 记录已放置的位置
...
###ai
                ...
                print(f"DEBUG: Placing {distractor_obj.name} at intended p: {p}, q: {q}")
                distractor_obj.set_pose(sapien.Pose(p, q))
                print(f"DEBUG: {distractor_obj.name} pose after set_pose: {distractor_obj.pose}")
                ...
                self._settle(0.5)
                print(f"DEBUG: {distractor_obj.name} pose after first settle: {distractor_obj.pose}")
            ...
            print("DEBUG: Final distractor poses after all settling:")
            for i, d_obj in enumerate(self.distractor_objs):
                print(f"  Distractor {i} ({d_obj.name}): {d_obj.pose}")
    ...
    def _load_model(self):
        ...
        if self.selected_distractor_model_ids is not None:
            for distractor_model_id, distractor_model_scale in zip(
                ...
                # 在 GraspSingleCustomInSceneEnv 类的 _load_model 方法中：
        # ... (加载 self.obj 的代码) ...

        # if self.selected_distractor_model_ids is not None:
        #     print(f"DEBUG: 开始加载干扰物循环。选中的干扰物ID: {self.selected_distractor_model_ids}")
        #     print(f"DEBUG: 选中的干扰物尺度: {self.selected_distractor_model_scales}")

        #     for i, (distractor_model_id, distractor_model_scale) in enumerate(zip(
        #         self.selected_distractor_model_ids,
        #         self.selected_distractor_model_scales,
        #     )):
        #         print(f"DEBUG: 循环迭代 {i}, 尝试加载干扰物: {distractor_model_id}，尺度: {distractor_model_scale}")
        #         try:
        #             distractor_obj = self._build_actor_helper(
        #                 distractor_model_id,
        #                 self._scene,
        ...
        #                 root_dir=self.asset_root,
        #             )
        #             print(f"DEBUG: _build_actor_helper 为 {distractor_model_id} 返回: {distractor_obj}")

        #             if distractor_obj is None:
        #                 print(f"错误: _build_actor_helper 为 {distractor_model_id} 返回了 None。跳过此干扰物。")
        #                 # 如果希望即使发生错误也尝试加载下一个，就用 continue
        #                 # continue 
        #                 # 如果希望一旦出错就停止加载后续干扰物，可以考虑用 break，但这可能不是我们想要的

        #             distractor_obj.name = distractor_model_id
        #             print(f"DEBUG: 为 {distractor_model_id} 设置名称为 {distractor_obj.name}")

        #             self.distractor_objs.append(distractor_obj)
        #             print(f"DEBUG: 已将 {distractor_model_id} 添加到 self.distractor_objs。当前数量: {len(self.distractor_objs)}")

        #         except Exception as e:
        #             print(f"错误: 加载干扰物 {distractor_model_id} 过程中发生异常: {e}")
        #             import traceback
        #             traceback.print_exc() # 打印完整的错误堆栈信息
        #             print(f"DEBUG: 由于错误，可能中断干扰物加载循环。")
        #             # 选择1: 如果希望任何错误都终止循环 (当前代码似乎是这样表现的)
        #             # break
        #             # 选择2: 打印错误后继续尝试加载下一个 (用于调试)
        #             print(f"DEBUG: 尽管发生错误，仍尝试加载下一个干扰物。")
        #             # continue # 确保即使一个失败了，也尝试下一个

        #     print(f"DEBUG: 干扰物加载循环结束。总共加载的干扰物数量: {len(self.distractor_objs)}")
...
# @register_env("GraspSingleOpenedCokeCanMyExpInScene-v0", max_episode_steps=80)
# class GraspSingleOpenedCokeCanMyExpInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
#     def __init__(self, **kwargs):
#         # if distractor_config == "less":
#         #     self.distractor_model_ids = [
#         #         "opened_pepsi_can",
...
#         # else:
#         #     raise NotImplementedError()
#         self.distractor_model_ids = ["apple", "bridge_spoon_generated_modified"]
#         kwargs["distractor_model_ids"] = self.distractor_model_ids
#         super().__init__(**kwargs)
...
@register_env("GraspSingleOpenedCokeCanMyExpInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanMyExpInSceneEnv(GraspSingleCustomOrientationInSceneEnv): # 继承自更灵活的父类
    def __init__(self, **kwargs):
        # 定义你的目标模型和干扰物模型
        self.target_model_id_for_task = "opened_coke_can"
        # self.my_custom_distractor_ids = ["apple", "bridge_spoon_generated_modified"]
        # self.my_custom_distractor_ids = ["bridge_spoon_generated_modified", "apple"]
        # self.my_custom_distractor_ids = ["bridge_carrot_generated_modified", "bridge_plate_objaverse_larger"]
        self.my_custom_distractor_ids = ["baked_green_cube_3cm", "baked_yellow_cube_3cm"]

        # 确保所有需要的模型ID都传递给基类以填充 model_db
        all_necessary_model_ids = [self.target_model_id_for_task] + self.my_custom_distractor_ids
        # 如果 kwargs 中已经有 model_ids，合并它们（去重）
        existing_model_ids = kwargs.pop("model_ids", [])
        kwargs["model_ids"] = list(set(all_necessary_model_ids + existing_model_ids))

        # 将你的自定义干扰物列表传递给 GraspSingleInSceneEnv 的逻辑
        kwargs["distractor_model_ids"] = self.my_custom_distractor_ids

        super().__init__(**kwargs)

        # print("--- Model DB Debug ---")
        # if "bridge_spoon_generated_modified" in self.model_db:
        #     print("Found 'bridge_spoon_generated_modified' in model_db.")
        #     print(self.model_db["bridge_spoon_generated_modified"])
        # else:
        #     print("'bridge_spoon_generated_modified' NOT FOUND in model_db. Available keys:", list(self.model_db.keys()))
        # print("----------------------")
    ...
    def reset(self, seed=None, options=None):
        ...
        # 在 reset 时明确指定目标模型和干扰物模型
        options["model_id"] = self.target_model_id_for_task # 确保目标是 coke can
        options["distractor_model_ids"] = self.my_custom_distractor_ids

        return super().reset(seed=seed, options=options)

    def get_language_instruction(self, **kwargs):
        # Put Carrot on Plate
        if "carrot" in self.my_custom_distractor_ids[0]:
            return "put carrot on plate"
        elif "cube" in self.my_custom_distractor_ids[0]:
            return "stack green cube on yellow cube"
...
class GraspSingleBridgeSpoonInSceneEnv(GraspSingleCustomOrientationInSceneEnv):
    def __init__(self, **kwargs):
        kwargs.pop("model_ids", None)
